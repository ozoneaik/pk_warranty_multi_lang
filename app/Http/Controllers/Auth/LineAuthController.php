...
class LineAuthController extends Controller
{
    ...
    {
        // ✅ รับ redirect จาก query โดยไม่ใช้ session
        $redirect = $request->query('redirect', '/dashboard');

        return Socialite::driver('line')
            ->with(['state' => base64_encode($redirect)]) // ✅ เก็บ redirect ใน state (ไม่หายบน iOS)
            ->scopes(['profile', 'openid', 'email'])
            ->redirect();
    }
    ...
    public function handleLineCallback(Request $request)
    {
        try {
            ...
            // ✅ ดึง redirect คืนจาก state แทน session
            $redirect = '/dashboard';
            if ($request->has('state')) {
                $redirect = base64_decode($request->get('state'));
            }
            ...
            // ✅ ส่งกลับไปหน้าที่ต้องการ (ฟอร์มรับประกัน)
            return redirect()->to($redirect);
        } catch (\Exception $e) {
            Log::error('LINE Login Error', [
                'message' => $e->getMessage(),
