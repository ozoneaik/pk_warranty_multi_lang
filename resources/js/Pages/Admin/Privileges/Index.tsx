        ...
        <AdminLayout
            header={
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 className="font-bold text-2xl text-gray-800 leading-tight">จัดการสิทธิพิเศษ</h2>
                        <p className="text-sm text-gray-500 font-normal mt-0.5">เพิ่ม ลบ แก้ไข และกำหนดเงื่อนไขของสิทธิพิเศษต่างๆ</p>
                    </div>
                </div>
            }
        >
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-gray-900">รายการสิทธิพิเศษทั้งหมด</h3>
                            <p className="text-sm text-gray-500">พบ {privileges.total || 0} รายการในระบบ</p>
                        </div>
                    </div>
                    <Link
                        href={route('admin.privileges.create')}
                        className="w-full md:w-auto bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg shadow-sm transition-all flex items-center justify-center gap-2 font-semibold text-sm whitespace-nowrap"
                    >
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-gray-50/80 border-b border-gray-100">
                                <tr>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider w-20 text-center">รูปภาพ</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">ชื่อสิทธิพิเศษ / รหัส</th>
                                    ...
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-center">การมองเห็น</th>
                                ...
                                {privileges.data.length > 0 ? privileges.data.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {item.image_url ? (
                                                <img src={item.image_url} alt={item.privilege_name} className="w-12 h-12 object-cover rounded-xl border border-gray-200 shadow-sm bg-white mx-auto" />
                                            ) : (
                                                <div className="w-12 h-12 bg-gray-100 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 text-[10px] font-medium shadow-sm mx-auto">
                                                    No Img
                                                </div>
                                            )}
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="font-bold text-gray-900 line-clamp-1">{item.privilege_name}</div>
                                            <div className="text-xs font-mono bg-purple-50 text-purple-700 px-2 py-0.5 rounded-md border border-purple-100 inline-block mt-1.5">
                                                {item.privilege_code}
                                            </div>
                                        </td>

                                        {/* Points */}
                                        <td className="px-6 py-4 text-center font-mono text-sm font-medium text-gray-500">
                                        ...
                                        <td className="px-6 py-4 text-center whitespace-nowrap">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold border 
                                                ${item.visibility_settings === 'both' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                                                    item.visibility_settings === 'user' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                                                        'bg-gray-50 text-gray-600 border-gray-200'}`}
                                            >
                                                {item.visibility_settings === 'both' ? 'Admin & User' : item.visibility_settings}
                                            </span>
                                    ...
                                    <tr>
                                        <td colSpan={8} className="px-6 py-12 text-center">
                                            ...
                                            <h3 className="text-sm font-medium text-gray-900">ไม่พบข้อมูลสิทธิพิเศษ</h3>
                                            <p className="mt-1 text-sm text-gray-500 mb-4">ยังไม่มีการสร้างสิทธิพิเศษในระบบ</p>
                                            <Link href={route('admin.privileges.create')} className="text-purple-600 hover:text-purple-700 font-medium text-sm underline underline-offset-4 decoration-purple-300">
                                                คลิกเพื่อสร้างรายการแรก
                                            </Link>
                                        </td>
                                    </tr>
                                )}
                    ...
                    {/* Pagination */}
                    {privileges.links && privileges.links.length > 3 && (
                        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                            <span className="text-sm text-gray-500 font-medium">
                                แสดง {privileges.from || 0} ถึง {privileges.to || 0} จากทั้งหมด {privileges.total || 0} รายการ
                            </span>
