...
// Lucide Icons
import { ArrowLeft, CheckIcon, CopyIcon, Package, DownloadIcon } from 'lucide-react';
...
export default function OrderReport({ orders, filters }: Props) {
    ...
    const handleReset = () => {
        setSearchValues({ search: '', status: 'all', start_date: '', end_date: '' });
        router.visit(route('admin.reports.orders.index'));
    };
    ...
    // Helper: Status Badge (Converted to Tailwind for consistency)
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'pending':
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200"><PendingIcon sx={{ fontSize: 14 }} /> รอตรวจสอบ</span>;
            case 'processing':
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200"><Package size={14} /> กำลังเตรียม</span>;
            case 'shipped':
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200"><ShippingIcon sx={{ fontSize: 14 }} /> จัดส่งแล้ว</span>;
            case 'completed':
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200"><CheckCircleIcon sx={{ fontSize: 14 }} /> สำเร็จ</span>;
            case 'cancelled':
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200"><CancelIcon sx={{ fontSize: 14 }} /> ยกเลิก</span>;
            default:
                return <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-700 border border-gray-200">{status}</span>;
        }
    };
    ...
    const CopyButton = ({ text }: { text: string }) => {
        ...
        return (
            <Tooltip title={copied ? "คัดลอกเรียบร้อย!" : "คัดลอกเลข Order"}>
                <button
                    onClick={handleCopy}
                    className={`ml-1.5 p-1 rounded-md transition-colors ${copied ? 'bg-emerald-100 text-emerald-600' : 'hover:bg-gray-100 text-gray-400 hover:text-gray-700'}`}
                >
                    {copied ? <CheckIcon size={14} /> : <CopyIcon size={14} />}
                </button>
            </Tooltip>
        ...
        <AdminLayout
            header={
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 className="font-bold text-2xl text-gray-800 leading-tight">จัดการคำสั่งซื้อ</h2>
                        <p className="text-sm text-gray-500 font-normal mt-0.5">ตรวจสอบและอัปเดตสถานะการแลกของรางวัล</p>
                    </div>
                </div>
            }
        >
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <div className="flex items-center gap-2 mb-4 text-gray-900">
                        ...
                        <span className="font-bold text-lg">ค้นหาและกรอง</span>
                    </div>
                    ...
                    <form onSubmit={handleSearch}>
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-end">
                            ...
                            <div className="md:col-span-5">
                                <label className="block text-sm font-semibold text-gray-700 mb-1.5">ค้นหา</label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <MagnifyingGlassIcon className="text-gray-400" fontSize="small" />
                                    </div>
                                    <input
                                        ...
                                        value={searchValues.search}
                                        onChange={(e) => setSearchValues({ ...searchValues, search: e.target.value })}
                                        className="pl-10 w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all text-sm py-2.5"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-4 flex gap-3">
                                <button
                                    type="submit"
                                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 px-4 rounded-lg text-sm font-semibold transition-colors shadow-sm flex items-center justify-center gap-2"
                                >
                                    <MagnifyingGlassIcon fontSize="small" />
                                    ค้นหา
                                </button>
                                {Object.values(searchValues).some(val => val !== '' && val !== 'all') && (
                                    <button
                                        type="button"
                                        onClick={handleReset}
                                        className="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 py-2.5 px-4 rounded-lg text-sm font-semibold transition-colors shadow-sm whitespace-nowrap"
                                    >
                                        ล้างค่า
                                    </button>
                                )}
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-gray-50/80 border-b border-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">วันที่สั่งซื้อ</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">ลูกค้า</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">สินค้าที่แลก</th>
                                    <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-center">แต้มที่ใช้</th>
                                ...
                                {orders.data.length > 0 ? (
                                        ...
                                        <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <span className="font-semibold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md text-sm border border-indigo-100">
                                                        {order.order_number}
                                                    </span>
                                                    <CopyButton text={order.order_number} />
                                                </div>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <div className="text-xs text-gray-500 mt-0.5">{order.phone_number}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="text-xs text-gray-400 font-mono mb-0.5">{order.product_code}</div>
                                                <div className="font-medium text-gray-800 line-clamp-1" title={order.product_name}>{order.product_name}</div>
                                                <div className="text-xs text-indigo-600 font-semibold mt-0.5">จำนวน: {order.quantity} รายการ</div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right space-x-1">
                                                <Tooltip title="เช็คสถานะจากระบบภายนอก">
                                                    <IconButton
                                                        ...
                                                        color="info"
                                                        onClick={() => {
                                                            router.post(route('admin.reports.orders.sync', order.id), {}, { preserveScroll: true });
                                                        }}
                                                        disabled={order.status === 'cancelled' || order.status === 'completed'}
                                                    >
                                                        <SyncIcon fontSize="small" />
                                    ...
                                    <tr>
                                        <td colSpan={7} className="px-6 py-12 text-center">
                                            ...
                                            <h3 className="text-sm font-medium text-gray-900">ไม่พบข้อมูลคำสั่งซื้อ</h3>
                                            <p className="mt-1 text-sm text-gray-500">ลองเปลี่ยนคำค้นหา หรือเลือกสถานะอื่น</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                >
                    <MenuItem onClick={() => handleSelectStatus('processing')} disabled={selectedOrder?.status === 'processing'} sx={{ fontSize: '14px', py: 1.5 }}>
                        <Package size={16} className="mr-3 text-blue-500" /> กำลังเตรียมสินค้า
                    </MenuItem>
                    <MenuItem onClick={() => handleSelectStatus('shipped')} disabled={selectedOrder?.status === 'shipped'} sx={{ fontSize: '14px', py: 1.5 }}>
                        <ShippingIcon fontSize="small" className="mr-3 text-indigo-500" /> จัดส่งแล้ว
                    </MenuItem>
                    <MenuItem onClick={() => handleSelectStatus('completed')} sx={{ fontSize: '14px', py: 1.5, color: 'success.main', fontWeight: 600 }}>
                        <CheckCircleIcon fontSize="small" className="mr-3" /> ทำรายการสำเร็จ (จบงาน)
                    </MenuItem>
                    <MenuItem onClick={() => handleSelectStatus('cancelled')} sx={{ fontSize: '14px', py: 1.5, color: 'error.main', fontWeight: 600 }}>
                        <CancelIcon fontSize="small" className="mr-3" /> ยกเลิก (คืนแต้ม)
                    </MenuItem>
                </Menu>
                ...
                >
                    <DialogTitle sx={{ fontWeight: 'bold' }}>ยืนยันการเปลี่ยนสถานะ</DialogTitle>
                    <DialogContent>
                        <Typography>
                            คุณต้องการเปลี่ยนสถานะ Order <span className="font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">{selectedOrder?.order_number}</span> เป็น
                            <strong className="text-gray-900"> {newStatus.toUpperCase()}</strong> ใช่หรือไม่?
                        </Typography>
                        {newStatus === 'cancelled' && (
                            <div className="mt-4 p-3 bg-rose-50 border border-rose-100 rounded-lg text-rose-600 text-sm font-medium flex gap-2 items-start">
                                <CancelIcon fontSize="small" className="mt-0.5" />
                                <span>หมายเหตุ: ระบบจะทำการคืนแต้มทั้งหมดเข้ากระเป๋าของลูกค้าโดยอัตโนมัติเมื่อกดยืนยัน</span>
                            </div>
                        )}
                    </DialogContent>
