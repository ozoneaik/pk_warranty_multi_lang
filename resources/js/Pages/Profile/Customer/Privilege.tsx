    ...
    return (
        ...
        >
            {products.map((item) => {
                ...
                const canRedeem = isPointEnough; // ปุ่มกดได้หรือไม่ขึ้นอยู่ที่คะแนน

                let buttonText = isPointEnough
                    ? t.Privilege.redeemReward
                    : t.Privilege.notEnoughPoints;
                                ...
                                <Button
                                    ...
                                    sx={{
                                        ...
                                        "&.Mui-disabled": {
                                            bgcolor: "#FF8A00",
                                            color: "white",
                                            opacity: 1, // ปรับให้ทึบ 100% เหมือนตอนปกติ
                                        },
                                        "&:hover": {
                                            bgcolor: "#FF8A00", // ใช้สีเดียวกับ bgcolor ตอนปกติ
                                        },
                                        transition: "0.2s",
                                    }}
                                    onClick={() => onRedeem(item)}
    ...
    return (
        ...
        >
            {products.map((item) => {
                ...
                // Privilege ไม่มี point ให้เช็ค แต่ถ้า claimed แล้วคือกดไม่ได้
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        +
                                        {item.earn_point?.toLocaleString() ?? 0}{" "}
                                        {t.Privilege.points}
                                    </Typography>
                                </Stack>
                                <Button
        ...
        >
            {products.map((item) => {
                ...
                if (isTierLocked) {
                    ...
                    buttonText = isPointEnough
                        ? t.Privilege.redeemCoupon
                        : t.Privilege.notEnoughPoints;
                }
...
export default function PrivilegePage() {
    ...
    const isNotExpired =
        tier_expired_at && dayjs().isBefore(dayjs(tier_expired_at));
    ...
    const [redeemStep, setRedeemStep] = React.useState<
        "detail" | "address_input" | "address_confirm" | "confirm" | "result"
    >("detail");
    const [selectedItem, setSelectedItem] = React.useState<ProductItem | null>(
        null,
    );
    const [loading, setLoading] = React.useState(false);
    const [mainDialog, setMainDialog] = React.useState<{
        open: boolean;
        data: any;
    ...
    React.useEffect(() => {
        if (addressForm.province && allAmphures.length > 0) {
            ...
            if (selectedProv) {
                setAmphures(
                    allAmphures.filter(
                        (a) => a.province_id === selectedProv.id,
                    ),
                );
            } else {
                setAmphures([]);
    ...
    React.useEffect(() => {
        if (addressForm.district && allTambons.length > 0) {
            const selectedAmp = amphures.find(
                (a) => a.name_th === addressForm.district,
            );
            if (selectedAmp) {
                setTambons(
                    allTambons.filter((t) => t.district_id === selectedAmp.id),
                );
            } else {
                setTambons([]);
            }
    ...
    const handleOpenRedeem = (item: ProductItem) => {
        ...
        if (item.delivery_type === "delivery") {
            setAddressForm({
                name: customer_info
                    ? `${customer_info.fname} ${customer_info.lname}`.trim()
                    : display_name,
    ...
    const getExpiryText = (item: ProductItem) => {
        ...
        if (item.expiry_type === "dynamic")
            return t.Privilege.limitedTime.replace(
                "{days}",
                String(item.expiry_days),
            );
        if (item.expired_at)
            return `${t.Privilege.validUntil}: ${dayjs(item.expired_at).format("D MMM YYYY")}`;
    ...
    return (
        <MobileAuthenticatedLayout title={t.Privilege.title}>
            ...
            <Container maxWidth={false} disableGutters sx={{ mt: 7.5, mb: 7 }}>
                ...
                <Box sx={{ px: isMobile ? 0.5 : 1.5, pt: 0.5 }}>
                    ...
                    >
                            ...
                            >
                                ...
                                <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                                    ...
                                    <Typography
                                        ...
                                        fontWeight={800}
                                        noWrap
                                        sx={{ lineHeight: 1.2, color: "#333" }}
                                    >
                                        {display_name}
                                    </Typography>

                                    {/* แถว: Point + คลิกเปิด Modal */}
                                    <Stack
                                        direction="row"
                                        alignItems="center"
                                        spacing={0.5}
                                    ...
                                    {/* {tier_expired_at && (
                                        ...
                                        >
                                            {t.Privilege.expiredAt}{" "}
                                            {dayjs(tier_expired_at).format(
                                                "D MMM",
                                            )}{" "}
                        ...
                        <Box
                            sx={{
                                background:
                                    "linear-gradient(180deg, #FFF8F0 0%, #FFE6CC 100%)", // พื้นหลังไล่เฉดสีครีมส้ม
                                p: 1,
                                position: "relative",
                            }}
                        >
                                    ...
                                    <span
                                        dangerouslySetInnerHTML={{
                                            __html: t.Privilege.collectMorePoints
                                                .replace(
                                                    "{point}",
                                                    `<b>${fmt.format(remainingPoints)}</b>`,
                                                )
                                                .replace("{tier}", nextTier),
                                        }}
                                    />
                                ) : (
                            ...
                            <Box sx={{ position: "relative", mx: 1, mb: 1 }}>
                                ...
                                <Box
                                    sx={{
                                        ...
                                        height: 6,
                                        background:
                                            "linear-gradient(90deg, #FF8A00 0%, #FF5500 100%)",
                                        borderRadius: 3,
                ...
                {tab === 2 && (
                    <RedeemHistory data={redeemHistory} orders={orders} />
                )}
                    ...
                    <DialogContent sx={{ px: 3, pb: 4, pt: 8 }}>
                            ...
                            <Box sx={{ textAlign: "center" }}>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        "privilege"
                                            ? `+${selectedItem.earn_point} Earn`
                                            : `${selectedItem.redeem_point} Points`}
                                    </Typography>
                                </Stack>
                        ...
                        {redeemStep === "address_input" && (
                            <Box>
                                <Typography
                                    ...
                                    mb={1}
                                >
                                    {t.Privilege.addressInputTitle}
                                </Typography>
                                <Stack spacing={2} mt={2}>
                                    <TextField
                                        label={
                                            t.Customer.form.firstname +
                                            " - " +
                                            t.Customer.form.lastname
                                        }
                        ...
                        {redeemStep === "address_confirm" && (
                            <Box sx={{ textAlign: "center" }}>
                                <Typography
                                    ...
                                    mb={3}
                                >
                                    {t.Privilege.addressConfirmTitle}
                                </Typography>
                                <Box
                                    sx={{
                                        textAlign: "left",
                        ...
                        {redeemStep === "confirm" && (
                            <Box sx={{ textAlign: "center" }}>
                                ...
                                <Chip
                                    label={
                                        selectedItem?.tier_level === 2
                                            ? "Gold"
                                            : selectedItem?.tier_level === 3
                                              ? "Platinum"
                                              : "Silver"
                                    }
                                    size="small"
                                ...
                                <Typography
                                    ...
                                    mb={3}
                                >
                                    {selectedItem?.product_type === "privilege"
                                        ? `+${selectedItem?.earn_point} ${t.Privilege.points}`
                                        : `${selectedItem?.redeem_point.toLocaleString()} ${t.Privilege.points}`}
                                </Typography>
                                <Button
                                    fullWidth
                                    variant="contained"
